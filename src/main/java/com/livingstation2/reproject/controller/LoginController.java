...
public class LoginController {
    ...
    @ResponseBody
    public ResponseEntity<LoginJsonResult> accountConfirm(@RequestBody TAccountEntity account, HttpServletRequest request , HttpServletResponse response) throws IOException {
        LoginJsonResult r = new LoginJsonResult();
        int flag = 0;
        //try中内容用来查找账号是否存在 ， 可以用账号或者邮箱登录

        TAccountEntity userAccount =null;

        try {
            String accountName = account.getAccountName();
            String accountMail = account.getUserEmailAddress();
            TAccountEntity accountGetByName = null;
            TAccountEntity accountGetByMail = null;
