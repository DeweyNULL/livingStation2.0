...
import com.livingstation2.reproject.bean.JsonResult;
import com.livingstation2.reproject.bean.UserCollectMiniType;
import com.livingstation2.reproject.bean.WebJsonResult;
...
public class WebCollectController {
    ...
    public ResponseEntity<WebJsonResult> addCollection(@RequestBody TUsercollectEntity userCollect, HttpServletRequest request , HttpServletResponse response){
        WebJsonResult r = new WebJsonResult();
        logger.info(userCollect.getWebApi());
        //对来存储的房间进行一次访问
        ...
        if(mapTypes.get("error").equals(0)){
            ...
            TUsercollectEntity tempUser = dyWebCollectService.getWebByUserAndWebSiteName(userCollect);
            if(tempUser!=null){
                r.setStatusCode(2);
                r.setResult("房间已经添加过了!");
            }else {
                dyWebCollectService.addOrUpdateUserCollect(userCollect);
                r.setStatusCode(0);
                //r.setResult("OK");
    ...
    public ResponseEntity<WebJsonResult> getWebCollection(@RequestBody UserCollectMiniType userCollectMiniType, HttpServletRequest request , HttpServletResponse response){
        ...
        List<TUsercollectEntity> userCollects =dyWebCollectService.getUserCollection(userCollectMiniType.getAccountName());
...
//                }
    ...
    public ResponseEntity<JsonResult> deleteWeb(@RequestBody TUsercollectEntity userCollect , HttpServletRequest request , HttpServletResponse response){
        logger.info(userCollect.toString());
        JsonResult result = new JsonResult();
